module.exports = (
  ...
  var score = [[null, null, null], [null, null, null], [null, null, null]];
  var will_end_game = false;

  client.on("messageReactionAdd", (reaction, user) => {
    if (
      reaction.message.id == new_message.id &&
      turn_id == user.id &&
      !will_end_game
    ) {
      let emoji;
      ...
      grid_message
        .edit(grid_message.content.replace(emoji, symbol))
        .then(new_mes => {
      ...
      if (
        didPlayerWin(symbols[0], player1_id) ||
        didPlayerWin(symbols[1], player2_id) ||
        didItTie()
      ) {
        will_end_game = true;
        return;
      ...
      let temp_message = new_message.content.replace(
        `<@${turn_id}>`,
        `<@${toggle_player(turn_id, player1_id, player2_id)}>`
      );
  ...
  function didPlayerWin(sym, player) {
    for (let i = 0; i < score.length; i++) {
      // Horizontal checks
      if (score[i][0] == sym && score[i][1] == sym && score[i][2] == sym) {
        new_message
          .edit(`**I win! :moneybag:  <@${player}> Congratulations!**`)
          .then(console.log("Successful win"))
          .catch(console.error);
        return true;
      }
      // Vertical checks
      else if (score[0][i] == sym && score[1][i] == sym && score[2][i] == sym) {
        new_message
          .edit(`**I win! :moneybag:  <@${player}> Congratulations!**`)
          .then(console.log("Successful win"))
          .catch(console.error);
    ...
    // Diagonal checks
    if (score[0][0] == sym && score[1][1] == sym && score[2][2] == sym) {
      new_message
        .edit(`**I win! :moneybag:  <@${player}> Congratulations!**`)
        .then(console.log("Successful win"))
        .catch(console.error);
      return true;
    } else if (score[0][2] == sym && score[1][1] == sym && score[2][0] == sym) {
      new_message
        .edit(`**I win! :moneybag:  <@${player}> Congratulations!**`)
        .then(console.log("Successful win"))
        .catch(console.error);
